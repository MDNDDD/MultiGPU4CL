...
CSR_graph<weight_type> toCSR(LDBC<weight_type> &graph, std::map<std::pair<int, int>, int > *edge_id = nullptr) {
    ...
    for (int i = 0; i < V; i++) {
        ...
        for (auto &xx : graph.OUTs[i]) {  
            ...
            ARRAY.ARRAY_source.push_back(i);
        }
    ...
    for (int i = 0; i < V; i++) {
        for (auto &xx : graph.OUTs[i]) {  
            ...
            ARRAY.ARRAY_inv.push_back(ARRAY.pointer[xx.first] ++);
        }
    }
    ...
    cudaMallocManaged(&ARRAY.inv, E_out * sizeof(int));
    cudaMemcpy(ARRAY.source, ARRAY.ARRAY_source.data(), E_out * sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(ARRAY.inv, ARRAY.ARRAY_inv.data(), E_out * sizeof(int), cudaMemcpyHostToDevice);
