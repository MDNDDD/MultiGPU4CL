...
// Optional: explicitly disable CUB_LOG level
#define CUB_LOG 0
...
//             }
//         }
//         // Check if the community size for the selected label has reached the maximum
//        //atomicAdd(&community_size[maxlabel], 1); // If the limit is not exceeded, increase the community size for the label
//        new_labels[tid] = maxlabel; // Record maxlabel
        
//     }
// }
__global__ void Get_New_Label(int *all_pointer, int *prop_labels, int *new_labels, int* community_size, int N, int MAX_GROUP_SIZE)
{
    int tid = blockDim.x * blockIdx.x + threadIdx.x; // Vertex index processed by the current thread
    if (tid < N) {
        ...
        for (int c = start; c < end; ) {
            ...
            int count = 1;
            c++;
            // Count consecutive occurrences of the same label
            while (c < end && prop_labels[c] == label) {
                count++;
                c++;
            }

            // Attempt to atomically increase community size
            bool assigned = false;
            while (!assigned) {
                int current_size = atomicAdd(&community_size[label], 0);
                if (current_size >= MAX_GROUP_SIZE) {
                    // Community is full, cannot assign this label
                    break;
                }
                int result = atomicCAS(&community_size[label], current_size, current_size + 1);
                if (result == current_size) {
                    // Successfully increased community size, assign this label
                    assigned_label = label;
                    assigned = true;
                    break;
                } else {
                    // Another thread updated the community size, retry
                }
            }
            if (assigned) {
                break; // Successfully assigned label, exit loop
            }
            // Otherwise, continue trying the next label
        }

        if (assigned_label == -1) {
            // Cannot assign any neighbor label, try assigning own label
            assigned_label = tid;
            bool assigned = false;
            while (!assigned) {
...
void CDLP_GPU(int N, CSR_graph<int>& input_graph, std::vector<int>& res, int MAX_GROUP_SIZE, int max_iterations = 100000)
{
    ...
    while (it < CD_ITERATION) // continue for a fixed number of iterations
    {
        ...
        cudaMemset(community_size, 0, N * sizeof(int));  // Reset community size at each iteration
