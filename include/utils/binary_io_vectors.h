#ifndef BINARY_IO_VECTORS_H
#define BINARY_IO_VECTORS_H
#pragma once
...
template<typename T>
void binary_save_vector_of_vectors(std::string path, std::vector<std::vector<T> >& myVector)
{
    ...
    for (auto& v : myVector) {
        ...
        // Store its contents
        FILE.write(reinterpret_cast<const char*>(&v[0]), v.size() * sizeof(T)); // T cannot be bool!
    }
    FILE.close();
}
...
#include <utils/binary_io_vectors.h>
