...
public:
    ...
    __host__ void set_nid (int distributed_graph_num, std::vector<std::vector<int> > graph_group) {
        Distributed_Graph_Num = distributed_graph_num;
        // long long *testma;
        // cudaMallocManaged(&nid, 10000000000ll * sizeof(long long*));
    ...
    __host__ void init_clean (int V, std::vector<std::vector<hop_constrained_two_hop_label>> &res, 
        CSR_graph<weight_type> &csr_graph, long long L_size, std::map<std::pair<int,int>,int> &edge_id) {
        cudaError_t err = cudaGetLastError();
        last_size = 1;
        ...
        long long pos = 1, mnsiz = V, mxsiz = 0;
        L_clean[0] = 0x7FFFFFFFFFFFFFFFLL;
        print_memory_info();
        for (long long i = 0; i < V; i ++) {
            L_start[i] = pos;
            for (int j = 0; j < res[i].size(); j ++) {
                // L_clean[pos] = _get_label(res[i][j].parent_vertex, res[i][j].hub_vertex, res[i][j].hop, res[i][j].distance);
                // pos ++;
                if (edge_id.count(std::make_pair(i, res[i][j].parent_vertex)) == 0) {
                    L_clean[pos] = _get_label(csr_graph.OUTs_Neighbor_start_pointers[i], 
                                                res[i][j].hub_vertex, res[i][j].hop, res[i][j].distance);
                    if ( res[i][j].hop != 0) {
                        printf("shit!!!!\n");
                    }
                    pos ++;
                    // if (i != res[i][j].parent_vertex)
                    //     printf("cant find %lld, %lld !!!\n", i, res[i][j].parent_vertex);
                } else {
                    L_clean[pos] = _get_label(edge_id[std::make_pair(i, res[i][j].parent_vertex)], 
                                                res[i][j].hub_vertex, res[i][j].hop, res[i][j].distance);
                    pos ++;
                    if (res[i][j].hop == 0) {
                        res[i][j].parent_vertex = i;
                    }
                }
                // if (res[i][j].parent_vertex < 0) {
                //     L_clean[pos] = _get_label(edge_id[std::make_pair(i, -res[i][j].parent_vertex - 1)], 
                //                               res[i][j].hub_vertex, res[i][j].hop, res[i][j].distance);
                //     pos ++;
                // } else {
                //     L_clean[pos] = _get_label(res[i][j].parent_vertex, res[i][j].hub_vertex, res[i][j].hop, res[i][j].distance);
                //     pos ++;
                // }
                // if (i != csr_graph.ARRAY_source[res[i][j].parent_vertex]) {
                //     L_clean[pos] = _get_label(edge_id[std::make_pair(res[i][j].parent_vertex, i)], 
                //                               res[i][j].hub_vertex, res[i][j].hop, res[i][j].distance);
                //     pos ++;
                //     // printf("i != res[i][j].to_vertex !!!\n");
                // } else {
                //     L_clean[pos] = _get_label(res[i][j].parent_vertex, res[i][j].hub_vertex, res[i][j].hop, res[i][j].distance);
                //     pos ++;
                // }
            ...
            mnsiz = min((long long)mnsiz, (long long)res[i].size());
            mxsiz = max((long long)mxsiz, (long long)res[i].size());
            // res[i].clear();
            // res[i].shrink_to_fit();
            // malloc_trim(0);
            // std::vector<hop_constrained_two_hop_label>().swap(res[i]);
            if (i % 200000 == 0) {
        ...
        L_start[V] = pos;
        // print_memory_info();
        // std::vector<std::vector<hop_constrained_two_hop_label>>().swap(res);
        // malloc_trim(0);
        // res.clear();
        // res.shrink_to_fit();
        printf("res capicity: %d\n", res.capacity());
        print_memory_info();
        printf("mnsiz, mxsiz: %lld, %lld\n", mnsiz, mxsiz);
    }
};
