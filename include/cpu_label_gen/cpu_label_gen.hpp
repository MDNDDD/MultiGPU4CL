#ifndef CPU_LABEL_GEN_H
#define CPU_LABEL_GEN_H
#pragma once

#include <cpu_label_gen/cpu_label_manager.hpp>
...
static void HSDL_with_large_queue_optimized (int v_k, int tag, int hop_now, int hop_cst) {	
	...
	for (auto &xx : L_temp_599[v_k]) {
		int L_vk_vertex = xx.hub_vertex;
		for (int k = xx.hop; k <= global_upper_k && Temp_L_vk[L_vk_vertex][k] != std::numeric_limits<int>::max(); ++ k) {
			Temp_L_vk[L_vk_vertex][k] = std::numeric_limits<int>::max();
		}
	}
	mtx_599[v_k].unlock();
...
static void _2023WWW_thread_function_optimized(int v_k) {
	...
	for (auto &xx : L_temp_599[v_k]) {
		int L_vk_vertex = xx.hub_vertex;
		auto& li = Temp_L_vk[L_vk_vertex];
		for (int k = xx.hop; k <= global_upper_k && li[k] != std::numeric_limits<int>::max(); k++) {
			li[k] = std::numeric_limits<int>::max();
		}
	}
	mtx_599[v_k].unlock();
...
static void hop_constrained_clean_L_distributed (hop_constrained_case_info_cpu &case_info,
...
/* canonical_repair_distributed */
static void hop_constrained_clean_L_distributed (hop_constrained_case_info_cpu &case_info,
vector<vector<hop_constrained_two_hop_label> >& LL, 
int start_vertex, int end_vertex, int thread_num) {
...
static void hop_constrained_clean_L_distributed (hop_constrained_case_info_cpu &case_info,
hop_constrained_case_info_gpu *info_gpu, vector<int>& nid_vec, int thread_num) {
	...
	for (auto &v: nid_vec) {
		...
		printf("%d nid_vec %d\n", v, v);
		results.emplace_back(pool.enqueue(
			[v, L, L_start, L_end, mark] { // pass const type value j to thread; [] can be empty
				for (long long i = L_start[v]; i < L_end[v]; i ++) {
					int hub_vertex = get_hub_vertex(L[i]);
					int hop = get_hop(L[i]);
					int distance = get_distance(L[i]);
					for (int j = L_start[hub_vertex]; j < L_end[hub_vertex]; j++) {
				...
				// for (long long i = L_start[v]; i < L_end[i]; i++) {
				// 	int u = _get_hubvertex(L[i]);
				// 	int u_hop = Lvi.hop;
...
/*canonical_repair*/
static void hop_constrained_clean_L (hop_constrained_case_info_cpu &case_info, vector<vector<hop_constrained_two_hop_label> >& LL, int thread_num, int N) {
	auto &L = LL;
	// int N = L.size();
	label_size_before_canonical_repair_599 = 0;
	label_size_after_canonical_repair_599 = 0;
...
static void hop_constrained_two_hop_labels_generation (graph_v_of_v<int> &input_graph, hop_constrained_case_info_cpu &case_info) {
...
static void hop_constrained_two_hop_labels_generation_init (graph_v_of_v<int> &input_graph, hop_constrained_case_info_cpu &case_info) {
...
, vector<vector<hop_constrained_two_hop_label> >&L, vector<int> &nid_vec) {
	...
	if (case_info.use_2023WWW_generation) {
		...
		printf("use_2023WWW_generation_optimized !!!\n");
		for (int v_k = 0; v_k < N; ++ v_k) {
			int x = nid_vec[v_k];
		...
		printf("use_GPU_version_generation !!!\n");
		boost::random::uniform_int_distribution<> qid_range{ static_cast<int>(0), static_cast<int>(num_of_threads_cpu - 1) };
		int tag = 0;
		for (int v_k = 0; v_k < N; ++ v_k) {
		...
		printf("use_GPU_version_generation_optimized !!!\n");
		boost::random::uniform_int_distribution<> qid_range{ static_cast<int>(0), static_cast<int>(num_of_threads_cpu - 1) };
		int tag = 0;
		for (int v_k = 0; v_k < N; ++ v_k) {
			T[tag][v_k].push_back(std::make_tuple(v_k, 0, v_k));
	...
	printf("Finished cpu generation!\n");
