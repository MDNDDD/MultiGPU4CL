...
// __device__ void insert_has_clean (long long* has_clean, long long d_input) {
...
// __global__ void clear_hash_clean (long long *has_clean, long long *d_input, long long inputSize, int hop_cst) {
...
// __device__ int query_hash_clean (long long *has_clean, long long d_input) {
//     long long pos = hash_pos_clean (d_input & (~0ull << 10));
//     long long old;
//     int cnt = 0;
//     while (true) {
...
// __global__ void init_hash_clean (long long *L, long long *has_clean, long long siz, int hop_cst) {
//     long long tid = blockIdx.x * blockDim.x + threadIdx.x;
//     if (tid < siz) {
//         long long LL = L[tid];
//         // printf("init_hash_clean LL: %lld\n", LL);
//         int hop = get_hop(LL);
//         for (int i = hop; i <= hop_cst; i ++) {
...
// long long *L_end, long long *has_clean, char *mark, long long clean_num, long long start) {
//     long long tid = blockIdx.x * blockDim.x + threadIdx.x;
//     if (tid >= clean_num) return;

//     int to_vertex = get_to_vertex(L_clean[tid]);
...
//         }
//     }
//     // int hub_vertex = get_to_vertex(L_clean[tid]);
//     // int to_vertex = get_hub_vertex(L_clean[tid]);
//     // int hop = get_hop(L_clean[tid]);
//     // int distance = get_distance(L_clean[tid]);
//     // if (hop == 0) return;
//     // for (long long i = L_start[hub_vertex]; i < L_end[hub_vertex]; i ++) {
//     //     if (hop > get_hop(L[i]) && (get_distance(L[i]) < distance) &&(get_hop(L[i]) && get_distance(L[i]) + 
//     //     query_hash_clean(has_clean, get_label(get_to_vertex(L[i]), to_vertex, hop - get_hop(L[i]), 0)) <= distance)
//     //     || (get_hop(L[i]) == 0 && query_hash_clean(has_clean, get_label(get_to_vertex(L[i]), to_vertex, hop - get_hop(L[i]) - 1, 0)) <= distance)
//     //     ) {
//     //         mark[start + tid] = 1;
//     //         return;
//     //     }
...
// // __global__ void clear_label_clean_balanced (
// //     long long *L_clean, long long *L, long long *L_start,
// //     long long *L_end, long long *has_clean, int *mark,
// //     long long clean_num, long long start)
// // {
// //     long long global_tid = blockIdx.x * blockDim.x + threadIdx.x;
// //     long long total_threads = gridDim.x * blockDim.x;

// //     // Each thread processes multiple (tid, i) pairs in a flattened manner
// //     for (long long tid = global_tid; tid < clean_num; tid += total_threads) {
// //         int to_vertex = get_to_vertex(L_clean[tid]);
// //         int hub_vertex = get_hub_vertex(L_clean[tid]);
// //         int hop = get_hop(L_clean[tid]);
// //         int distance = get_distance(L_clean[tid]);
...
// //         long long start_idx = L_start[hub_vertex];
// //         long long end_idx = L_end[hub_vertex];

// //         // Iterate over all L[i] entries for this hub_vertex, in a flattened way
// //         for (long long i = start_idx + threadIdx.y; i < end_idx; i += blockDim.y) {
// //             int l_hop = get_hop(L[i]);
// //             int l_distance = get_distance(L[i]);
// //             int l_hub = get_hub_vertex(L[i]);

// //             if (l_hop && (hop > l_hop)) {
// //                 int label = get_label(to_vertex, l_hub, hop - l_hop, 0);
// //                 int d = query_hash_clean(has_clean, label);
// //                 if (l_distance + d <= distance) {
// //                     atomicOr(&mark[start + tid], 1);
// //                     break;
// //                 }
// //             } else if (l_hop == 0) {
// //                 int label = get_label(to_vertex, l_hub, hop - 1, 0);
// //                 int d = query_hash_clean(has_clean, label);
// //                 if (d <= distance) {
// //                     atomicOr(&mark[start + tid], 1);
// //                     break;
...
// //                                     long long *L_end, long long *has_clean, int *mark, 
// //                                     long long clean_num, long long start) {
// //     // Each block processes one L_clean entry (one tid)
...
// //     // Shared memory to coordinate early exit
...
// //     long long start_idx = L_start[hub_vertex];
// //     long long end_idx = L_end[hub_vertex];
// //     long long N = end_idx - start_idx;
    
// //     // Block-stride loop pattern for load balancing
// //     for (long long i = threadIdx.x; i < N && !found; i += blockDim.x) {
// //         long long global_i = start_idx + i;
        
// //         // Check condition
// //         bool condition = false;
// //         if (hop > get_hop(L[global_i])) {
// //             if (get_hop(L[global_i]) != 0) {
// //                 condition = (get_distance(L[global_i]) + 
// //                             query_hash_clean(has_clean, get_label(to_vertex, get_hub_vertex(L[global_i]), 
// //                             hop - get_hop(L[global_i]), 0)) <= distance);
// //             } else {
// //                 condition = (query_hash_clean(has_clean, get_label(to_vertex, get_hub_vertex(L[global_i]), 
// //                             hop - get_hop(L[global_i]) - 1, 0)) <= distance);
// //             }
// //         }
        
// //         if (condition) {
// //             // Only one thread should set the mark
...
//     long long tid = blockIdx.x * blockDim.x + threadIdx.x;
//     if (tid <= TABLE_SIZE_CLEAN_MINUS_ONE) {
//         if (has_clean[tid]) printf("error !!!!!\n");
//     }
// }

// __global__ void update_L (long long *L, long long *L_start, long long *L_end, 
// long long start_pos, long long L_size) {
//     long long tid = blockIdx.x * blockDim.x + threadIdx.x;
//     if (tid < L_size) {
//         long long LL = L[tid];
//         if (get_to_vertex(LL) != get_to_vertex(L[tid - 1])) {
//             L_start[get_to_vertex(LL)] = start_pos + tid;
//         }
//         if (get_to_vertex(LL) != get_to_vertex(L[tid + 1])) {
//             L_end[get_to_vertex(LL)] = start_pos + tid + 1;
//         }
//     }
// }

// void gpu_clean_v3 (long long L_clean_start, long long L_clean_end, 
...
//     long long last_size = info_gpu->last_size;
//     char *mark = info_gpu->mark;
//     err = cudaGetLastError();
//     if (err != cudaSuccess) {
//         printf("Kernel launch error clean 0: %s\n", cudaGetErrorString(err));
//         exit(0);
//     }
...
//     // printf("Blocks_num: %lld\n", BLOCKS_NUM);
    
//     init_hash_clean<<<BLOCKS_NUM, THREADS_NUM_CLEAN>>>(L + L_clean_start, has_clean, clean_num, hop_cst);
//     cudaDeviceSynchronize();
//     err = cudaGetLastError();
//     if (err != cudaSuccess) {
//         printf("Kernel launch error clean 1: %s\n", cudaGetErrorString(err));
//         exit(0);
//     }
//     // printf("init_hash_clean done !!!\n");
...
//     clear_label_clean<<<BLOCKS_NUM, THREADS_NUM_CLEAN>>>(L + L_clean_start, L, L_start, L_end, has_clean, mark, clean_num, L_clean_start);
//     // clear_label_clean<<<clean_num, 512>>>(L + L_clean_start, L, L_start, L_end, has_clean, mark, clean_num, L_clean_start);
//     // dim3 blockDim(32, 8);  // 256 threads per block
//     // dim3 gridDim((clean_num + blockDim.x - 1) / blockDim.x);
//     // clear_label_clean_balanced<<<gridDim, blockDim>>>(L + L_clean_start, L, L_start, L_end, has_clean, mark, clean_num, L_clean_start);
//     cudaDeviceSynchronize();
//     err = cudaGetLastError();
//     if (err != cudaSuccess) {
//         printf("Kernel launch error clean 2: %s\n", cudaGetErrorString(err));
//         exit(0);
//     }
//     // printf("clear_label_clean done !!!\n");
    
//     clear_hash_clean<<<BLOCKS_NUM, THREADS_NUM_CLEAN>>>(has_clean, L + L_clean_start, clean_num, hop_cst);
//     cudaDeviceSynchronize();
//     err = cudaGetLastError();
//     if (err != cudaSuccess) {
//         printf("Kernel launch error clean 3: %s\n", cudaGetErrorString(err));
//         exit(0);
//     }
//     // printf("clear_hash_clean done !!!\n");
...
//     update_L<<<BLOCKS_NUM, THREADS_NUM_CLEAN>>>(L + last_size, L_start, L_end, last_size, *d_num_selected);
