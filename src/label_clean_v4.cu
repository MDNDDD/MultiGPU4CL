...
__device__ long long change_label_ (long long x, int* source) {
...
__device__ long long get_source (long long to_vertex, int *source) {
...
__global__ void update_L (long long *L, long long *L_start, long long *L_end, int *source,
    ...
    if (tid < L_size) {
        long long LL = L[tid];
        if (get_source(LL, source) != get_source(L[tid - 1], source)) {
            L_start[get_source(LL, source)] = start_pos + tid;
        }
        if (get_source(LL, source) != get_source(L[tid + 1], source)) {
            L_end[get_source(LL, source)] = start_pos + tid + 1;
        }
    }
}
...
void gpu_clean_v4 (CSR_graph<weight_type>& input_graph, long long L_clean_start, long long L_clean_end, 
    ...
    // printf("L_start[L_clean_start]: %lld\n", L_clean_start);
    last_pos = last_size = L_clean_start;
    info_gpu->last_size = last_size;
    ...
    err = cudaGetLastError();
    if (err != cudaSuccess) {
        printf("Kernel launch error clean 4: %s\n", cudaGetErrorString(err));
        exit(0);
    }
