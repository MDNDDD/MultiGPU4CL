...
__forceinline__ __device__ void mod_hash_inf (int *has, int *nid, int *vid, int hub, int tar, int hop, int hop_cst, int value) {
    long long index1 = (long long) vid[hub] * VV * (hop_cst + 1);
    has[index1 + tar * (hop_cst + 1) + hop] = value;
}
__forceinline__ __device__ void mod_hash (int *has, int *nid, int *vid, int hub, int tar, int hop, int hop_cst, int value) {
    long long index1 = (long long) vid[hub] * VV * (hop_cst + 1);
    // has[index1 + tar * (hop_cst + 1) + hop] = value;
    atomicMin(&has[index1 + tar * (hop_cst + 1) + hop], value);
}
__forceinline__ __device__ int get_hash (int *has, int *nid, int *vid, int hub, int tar, int hop, int hop_cst) {
    long long index1 = (long long) vid[hub] * VV * (hop_cst + 1);
    return has[index1 + tar * (hop_cst + 1) + hop];
}
__forceinline__ __device__ int mod_get_hash (int *has, int *nid, int *vid, int hub, int tar, int hop, int hop_cst, int value) {
    long long index1 = (long long) vid[hub] * VV * (hop_cst + 1);
    int idx = (int)(index1 + tar * (hop_cst + 1) + hop);

    int old_val = atomicMin(&has[idx], value);
    if (old_val > 1e7) {
        return 1;
    }
    return 0;
    // if (has[index1 + tar * (hop_cst + 1) + hop] > 1e7) {
    //     has[index1 + tar * (hop_cst + 1) + hop] = value;
    //     return 1;
    // } else if (has[index1 + tar * (hop_cst + 1) + hop] > value) {
    //     has[index1 + tar * (hop_cst + 1) + hop] = value;
    // }
    // return 0;
}
...
__global__ void clearKernel_das (long long *d_table, long long *d_input, int *nid, int *vid, int *source, int inputSize) {
    long long idx = threadIdx.x + blockIdx.x * blockDim.x;
    if (idx >= inputSize) return;

    long long item = change_label(d_input[idx], source);
    d_input[idx] = (d_input[idx] & (~0ull << 10))
                 + get_hash ((int*)d_table, nid, vid, get_hub_vertex(item), get_to_vertex(item), 0, 0);
    mod_hash_inf ((int*)d_table, nid, vid, get_hub_vertex(item), get_to_vertex(item), 0, 0, INT_MAX);
}

__global__ void clearKernel_has (long long *d_table, long long* d_input, int *nid, int *vid, int *source, int inputSize, int hop_cst) {
    long long idx = threadIdx.x + blockIdx.x * blockDim.x;
    if (idx >= inputSize) return;
    ...
    for (int i = hop; i <= hop_cst; i ++) {
        mod_hash_inf ((int*)d_table, nid, vid, get_hub_vertex(item), get_to_vertex(item), i, hop_cst, INT_MAX);
    }
}

template<int THREADS_NUM>
__global__ void HSDL_gather_kernel (long long *T_pre, unsigned long long *T_pre_offset, long long *T_after, unsigned long long *T_after_offset, int *nid, int *vid,
    ...
    while (cta_offset < T_pre_offset[0]) {
        ...
        while (__syncthreads_or(row_end - row_begin >= THREADS_NUM)) {
            ...
            while (__syncthreads_or(gather < gather_end)) {
                if (gather < gather_end) {
                    ...
                    // mod_get_hash (int *has, int *nid, int *vid, int hub, int tar, int hop, int hop_cst, int value)
                    if (hub_vertex < out_edge[gather] && mod_get_hash((int*)das, nid, vid, hub_vertex, out_edge[gather], 0, 0, get_distance(u) + out_edge_weight[gather])) {
                        thread_data_in = 1;
                    } else {
                        thread_data_in = 0;
                    }
        ...
        while (cta_progress < total) {
            ...
            if (thread_id < min(remain, THREADS_NUM)) {
                ...
                if (hub_vertex < out_edge[e] && mod_get_hash((int*)das, nid, vid, hub_vertex, out_edge[e], 0, 0, get_distance(u) + out_edge_weight[e])) {
                    thread_data = 1;
                } else {
                    thread_data = 0;
                }
...
__global__ void Tranverse_kernel (long long *T1, long long *T2, long long *has, int *nid, int *vid, long long *T_offset_begin, long long *T_offset_end,
        int *source, char *flag, unsigned long long *T_size, int V, int hop_cst) {
    ...
    for (int i = 1; i < hop; i ++) {
        ...
        for (long long j = posl; j < posr; j ++) {
            ...
            // get_hash (int *has, int *nid, int *vid, int hub, int tar, int hop, int hop_cst) // tag_hash
            // get_hash(hash, nid, vid, hub_vertex, mid_vertex, hop_cst, hop - i)
            // if (mid_vertex == 1 && hub_vertex == 1) {
            //     printf("check: %d, %d, %d\n", to_vertex, hop, dis);
            //     printf("check: %d, %d, %d\n", i, get_distance(TT), get_hash((int*)has, nid, vid, mid_vertex, hub_vertex, hop - i, hop_cst));
            // }
            if (get_distance(TT) + get_hash((int*)has, nid, vid, mid_vertex, hub_vertex, hop - i, hop_cst) <= dis) {
                flag[tid] = 0;
                return;
            }
...
__global__ void updateHas (long long *T, long long T_start, long long *has, int *nid, int *vid, unsigned long long T_size, 
    ...
    for (int i = hop; i < hop_cst; i ++) {
        mod_hash ((int*)has, nid, vid, get_hub_vertex(TT), get_to_vertex(TT), get_hop(TT), hop_cst, get_distance(TT)); // tag_hash
        // printf("updateHas: %d, %d, %d, %d\n", get_hub_vertex(TT), get_to_vertex(TT), get_hop(TT), get_distance(TT));
...
__global__ void init_T (int group_size, long long *T, long long *has, int *nid, int *vid, long long *T_offset_begin, long long *T_offset_end, 
    ...
    for (int i = 0; i <= hop_cst; i ++) {
        mod_hash ((int*)has, nid, vid, v, v, i, hop_cst, 0); // tag_hash
        // mod_hash (int *has, int hub, int tar, int hop, int hop_cst, int value)
        TT += (1 << 10);
    }
...
__global__ void check_hash_(long long *h_table) {
    long long tid = blockIdx.x * blockDim.x + threadIdx.x;
    if (tid <= TABLE_SIZE_MINUS_ONE && h_table[tid] < 1e8) {
...
void label_gen_v4 (CSR_graph<weight_type>& input_graph, hop_constrained_case_info_v2 *info, long long *L, 
    ...
    cudaMemcpyToSymbol(VV, &V, sizeof(int));
    ...
    for (int hop = 1; hop <= hop_cst; hop ++) { // generate the label with (hop = hop)
        ...
        HSDL_gather_kernel<THREADS_NUM> <<<BLOCKS_NUM, THREADS_NUM>>>(T + last_pos, T_pre_offset, T + last_pos + last_size[0], 
            T_after_offset, nid, vid, D, out_edge, out_edge_weight, out_pointer, inv, source, hop);
        cudaDeviceSynchronize();
        ...
        if (last_size[0] == 0) break;
        BLOCKS_NUM = CALC_BLOCKS_NUM_NOLIMIT(THREADS_NUM, last_size[0]);
        clearKernel_das <<<BLOCKS_NUM, THREADS_NUM>>> (D, T + last_pos, nid, vid, source, last_size[0]);
        cudaDeviceSynchronize();
        CHECK_CUDA_KERNEL();
        // printf("clear done.\n");
        ...
        // step3 tranverse
        BLOCKS_NUM = CALC_BLOCKS_NUM_NOLIMIT(THREADS_NUM, last_size[0]);
        Tranverse_kernel <<<BLOCKS_NUM, THREADS_NUM>>> (T + last_pos, T, H, nid, vid, T_offset_begin, T_offset_end, source, flag, T_after_offset, V, hop_cst);
        cudaDeviceSynchronize();
        CHECK_CUDA_KERNEL();
        ...
        // step5 update hash
        updateHas<<<BLOCKS_NUM, THREADS_NUM>>>(T + last_pos, last_pos, H, nid, vid, last_size[0], T_offset_begin, T_offset_end, source, V, hop, hop_cst);
        cudaDeviceSynchronize();
        CHECK_CUDA_KERNEL();
        // printf("updatehash done.\n");

        check_hash_<<<BLOCKS_NUM, THREADS_NUM>>>(D);
        cudaDeviceSynchronize();
        CHECK_CUDA_KERNEL();
    }
    last_pos += last_size[0] - 1;
    BLOCKS_NUM = CALC_BLOCKS_NUM_NOLIMIT(THREADS_NUM, last_pos);
    clearKernel_has<<<BLOCKS_NUM, THREADS_NUM>>>(H, T + 1, nid, vid, source, last_pos, hop_cst);
    cudaDeviceSynchronize();
    CHECK_CUDA_KERNEL();
